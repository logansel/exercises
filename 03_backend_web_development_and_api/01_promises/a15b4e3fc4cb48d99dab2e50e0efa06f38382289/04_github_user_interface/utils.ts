...
export const getOneRepoInfos = (repoUrl: string): Promise<Repo> => {
  return fetch(repoUrl).then((response: Response) => response.json());
};

export const getReposUrlByNickname = (nickname: string): Promise<string> => {
...
export const listRepos = (reposUrl: string): Promise<Repo[]> => {
  return fetch(reposUrl).then((response: Response) => response.json());
};
